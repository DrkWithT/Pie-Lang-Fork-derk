...
#include "../ExpVM/Compiler.hxx"
#include "../ExpVM/Functions.hxx"
...
namespace pie {
    namespace vm {
        ...
        [[nodiscard]] bool Compiler::emit_expr(const expr::ExprPtr& any_expr) {
            ...
            if (const auto expr_bool = std::get_if<expr::Bool*>(&expr_variant); expr_bool) {
                return emit_bool(*expr_bool);
            } else if (const auto expr_num = std::get_if<expr::Num*>(&expr_variant); expr_num) {
                return emit_num(*expr_num);
            } else if (const auto expr_str = std::get_if<expr::String*>(&expr_variant); expr_str) {
                return emit_string(*expr_str);
            } else if (const auto expr_name = std::get_if<expr::Name*>(&expr_variant); expr_name) {
                return emit_name(*expr_name);
            } else if (const auto expr_assign = std::get_if<expr::Assignment*>(&expr_variant); expr_assign) {
                return emit_assignment(*expr_assign);
            } else if (const auto expr_call = std::get_if<expr::Call*>(&expr_variant); expr_call) {
                return emit_call(*expr_call);
            } else if (const auto expr_closure = std::get_if<expr::Closure*>(&expr_variant); expr_closure) {
                return emit_closure(*expr_closure);
            } else if (const auto expr_prefix = std::get_if<expr::Prefix*>(&expr_variant); expr_prefix) {
                return emit_prefix(*expr_prefix);
            } else if (const auto expr_infix = std::get_if<expr::Infix*>(&expr_variant); expr_infix) {
                return emit_infix(*expr_infix);
            } else if (const auto expr_unary_op = std::get_if<expr::UnaryOp*>(&expr_variant); expr_unary_op) {
                return emit_unary_op(*expr_unary_op);
            } else if (const auto expr_binary_op = std::get_if<expr::BinOp*>(&expr_variant); expr_binary_op) {
